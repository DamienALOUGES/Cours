...
# version matricielle
require(dummies)
#load("ttfull.rda") fichier Ryan
ttMatrice<-as.matrix(dummy.data.frame(ttfull))


# modelisation ####
# regression lineaire - joué 100 fois pour évaluer la performance####
# perfRL<-c() ; 
for (i in 1:100) {
...
valid<-sample(nrow(ttMatrice),floor(nrow(ttMatrice)/3)) 
arbre<-rpart(SalePrice~.,ttfull[-valid],control=rpart.control(cp=0.001,minsplit=2,minbucket=1,maxdepth=30)) ;
rpart.plot(arbre,cex=0.01)
bestCP= arbre$cptable[which.min(arbre$cptable[,"xerror"]),"CP"]
arbrePenalise<-prune(arbre,cp=bestCP)
predAbre<-predict(arbre,ttfull[valid])
...
bestCP= arbre$cptable[which.min(arbre$cptable[,"xerror"]),"CP"]
arbrePenalise<-prune(arbre,cp=bestCP)
perfAbrePenal=c() ;for (i in 1:1000) {
  valid<-sample(nrow(ttfull),floor(nrow(ttfull)/3)) ;# test<-sample(nrow(tt[-valid]),floor(nrow(tt)/10))
  predAbrePenal<-predict(arbrePenalise,ttfull[valid])
  perfAbrePenal<-c(perfAbrePenal,R2(ttfull[valid]$SalePrice,predAbrePenal))
}
save(perfAbrePenal,file="perfarbre.rda")
ggplot(data.table(perfArbre=perfAbrePenal),aes(x=perfArbre))+geom_histogram(binwidth = 0.0015)+theme_tufte()

# randomForest ####
require(randomForest)
# reglageRF<-data.table() ; 
n<-0 ; for (nodesize in 1:15) {
   for (mtry in 1:60) {n<-n+1 ; 
    rF<-randomForest(SalePrice~.,data=ttfull[-valid],mtry=mtry,ntree=250,nodesize=nodesize)
    predRF<-predict(rF,ttfull[valid])
    perf=sqrt(sum((predRF-ttfull[valid]$SalePrice)^2)/nrow(ttfull[valid]))
    reglageRF<-rbind(reglageRF,t(c(mtry,nodesize,perf)))
    print(c(n,mtry,nodesize,perf))
}} ; 

reglageRF2<-copy(reglageRF) ; setnames(reglageRF2,c("mtry","nodesize","perf"))
#save(reglageRF,file="reglageRF.rda")#reglageRFnodesize ; reglageRFmtry
ggplot(reglageRF2[nodesize==1],aes(x=mtry,y=perf))+geom_point()+theme_tufte()+geom_smooth(method="loess",span=0.2)+ylim(0.047,0.053)+xlim(0,60)
# perfRF<-c() ; 
for (i in 1:1000) {
  valid<-sample(nrow(ttfull),floor(nrow(ttfull)/3)) ;# test<-sample(nrow(tt[-valid]),floor(nrow(tt)/10))
  rF<-randomForest(SalePrice~.,data=ttfull[-valid],mtry=40,ntree=250,nodesize=7)
  predRF<-predict(rF,ttfull[valid])
  perf=R2(ttfull[valid]$SalePrice,predRF)
  perfRF<-c(perfRF,perf)
}  
ggplot(data.table(perfRF=perfRF),aes(x=perfRF))+geom_histogram(binwidth = 0.0015)+theme_tufte()
save(reglageRF,reglageRF2,perfArbre,perfRLEN,perfRL,perfRF,file="perf.rda")
...
gbm.x <- wich(names(ttfull)!="SalesPrice")
gbm.y <- wich(names(ttfull)=="SalesPrice")
valid <- 

mod.gbm <- gbm(SalePrice~., distribution = "gaussian",data = ttfull[-valid], n.trees = 10000,interaction.depth = TC, n.minobsinnode = n.minobsinnode, shrinkage = shrinkage,bag.fraction = 0.8, cv.folds = 3,train.fraction = 0.7)
pred.gbm<-predict.gbm(mod.gbm,n.trees=gbm.perf(mod.gbm),ttfull[valid])#mod.gbm$n.trees
  


#xgboost
require("xgboost")
indexTarget<-grep("SalePrice",colnames(ttMatrice))
valid   <- sample(nrow(ttMatrice),floor(nrow(ttMatrice)/3)) 

dtrain <- xgb.DMatrix(data = ttMatrice[-valid,-indexTarget], label = ttMatrice[-valid,"SalePrice"]) ;  
dvalid <- xgb.DMatrix(data = ttMatrice[valid,-indexTarget], label = ttMatrice[valid,"SalePrice"]) ;

xgb.params <- list("objective"  = "reg:squarederror", "eta" = 1, "subsample" = 0.9, "colsample_bytree" = mtry, "max_depth" = 100, "gamma" = gamma,"lambda"=lambda)
